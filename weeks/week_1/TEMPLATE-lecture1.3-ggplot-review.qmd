...
#| warning: false
# ~ CODE ALONG HERE ~
# scaling - process of separating out observations, ex: points by species using color
...
ggplot(penguins, aes(bill_length_mm, bill_depth_mm, color = body_mass_g)) + 
  geom_point() + 
  scale_color_gradient(low = 'darkblue', high = 'yellow') # assign color gradient
```
...
ggplot() +
  geom_point(data = penguins, 
             mapping = aes(bill_length_mm, bill_depth_mm, color = species)) + 
  geom_point(data = penguins_summary,
             mapping = aes(mean_bill_length_mm, 
                           mean_bill_depth_mm, 
                           color = species, 
                           fill = species),
...
# ~ CODE ALONG HERE ~
ggplot(penguins, aes(bill_length_mm, bill_depth_mm)) + 
  geom_point(aes(color = species)) + # to get lm for whole data, not just species
  geom_smooth(method = 'lm')
```
...
ggplot(penguin_summary, aes(species, n)) + 
...
# ~ CODE ALONG HERE ~
ggplot(penguins, aes(x = species, fill = island)) + 
  geom_bar(position = 'fill') # fill out graph to compare proportions
```
...
# ~ CODE ALONG HERE ~
ggplot(penguins, aes(x = species, fill = island)) + 
  geom_bar(position = 'dodge') # or set all bars next to each other
```
...
# ~ CODE ALONG HERE ~
ggplot(penguins, aes(x = species, fill = island)) + 
  geom_bar(position = position_dodge2(preserve = 'single')) # function is a fancier way of doing it with more options
```
...
# coord system is by default a cartesian coord system,
...
ggplot(penguins, aes(x = species)) +
  geom_bar() + 
  theme_light() + 
  theme(axis.title = element_text(size = 25, color = 'purple'))
```
...
ggplot(penguins, aes(x = flipper_length_mm, fill = species)) + 
  ...
  labs(x = 'Fliper Length (mm)', y = 'Frequency', 
       fill = 'Species',
